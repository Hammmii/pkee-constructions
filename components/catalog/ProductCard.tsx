    ...
    <CursorPreview src={src} alt={alt}>
      ...
      >
        <div className="relative aspect-[4/5] w-full overflow-hidden bg-stone/40">
          {src ? (
            /* Shared-element morph → PDP hero (R2.1): the browser animates
               this card image into the PDP hero, which carries the same
               `pkee-product-{slug}` name. */
            <ViewTransition name={`pkee-product-${product.slug}`} default="none" share="morph">
              <Image
                src={src}
                alt={alt}
                fill
